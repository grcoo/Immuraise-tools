        ...
        if (interaction.commandName === commands_1.command) {
            ...
            if (interaction.options.getSubcommand() === commands_1.subCommands.deal) {
                ...
                if (pt !== null) {
                    ...
                    yield interaction.reply((0, embeds_1.successEmbedsWithDescription)(`:confetti_ball: ${pt.name} 清算！`, fields, `:moneybag: 合計: ${(silver / 1000000).toFixed(2)}M 修理費以外: ${((silver - totalCost) / 1000000).toFixed(2)}M 分配金: × 0.8 = ${(substanceSilver / 1000000).toFixed(2)}M \n:crossed_swords:平均IP: ${Math.floor(totalIp / pt.list.length)} \n:family_mmgb:参加人数: ${pt.list.length}名`));
                    yield ptList.delete(ptname);
                }
                else {
                    yield interaction.reply((0, embeds_1.dangerEmbeds)(`pt: ${ptname}は未登録です。`));
